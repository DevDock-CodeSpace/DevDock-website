...
- **Sidebar limits:** each type section shows at most 3 items (`pickSidebarItems`): your **pins** first (`workspace_pins`, private per user), then recently opened (localStorage, `features/workspaces/recent.ts`), falling back to recently updated. The open workspace always shows. "All courses · N" links to Home `?type=`. Long names use `TruncatedText` (tooltip only when cut off).
